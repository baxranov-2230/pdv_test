...
def generate_group_results_pdf(group_id: str, results: List[Dict]) -> BytesIO:
    ...
    if not results:
        ...
        no_data = Paragraph(
            "Ushbu guruh uchun test natijalari topilmadi.", styles["Normal"]
        )
        elements.append(no_data)
    else:
        # Jadval ma'lumotlarini tayyorlash
        table_data = [["№", "Talaba", "Test", "Ball (foiz)", "Sana", "Holat"]]
            ...
            if taken_at:
                try:
                    date_obj = datetime.fromisoformat(
                        str(taken_at).replace("Z", "+00:00")
                    )
                    taken_at = date_obj.strftime("%d.%m.%Y %H:%M")
                except:
                    taken_at = str(taken_at)[:16]

            # Paragraph obyektlari bilan uzun matnlarni wrap qilish
            table_data.append(
                [
                    str(idx),
                    Paragraph(result.get("student_name", "N/A"), cell_style),
                    Paragraph(result.get("test_title", "N/A"), cell_style),
                    f"{score:.1f}%",
                    taken_at,
                    status,
                ]
        ...
        # Jadval yaratish - ustun kengliklarini moslashtirilgan
        table = Table(
            table_data,
            colWidths=[
                0.5 * inch,
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (-1, -1),
                        "TOP",
                    ),  # Vertical align top for wrapped text
        ...
        elements.append(
            Paragraph(
                f"• O'tganlar: {passed_count} ({passed_count/len(results)*100:.1f}%)",
                stats_style,
            )
        )
        elements.append(
            Paragraph(
                f"• O'tmaganlar: {failed_count} ({failed_count/len(results)*100:.1f}%)",
                stats_style,
            )
        )
